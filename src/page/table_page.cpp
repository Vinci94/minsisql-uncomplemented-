...
bool TablePage::InsertTuple(Row &row, Schema *schema, Txn *txn, LockManager *lock_manager, LogManager *log_manager) {
  ...
  // LOG(INFO) <<"GeTablePageId: " <<GetTablePageId()<<" GetprePageId: " << GetPrevPageId()<<" GetNextPageId: " << GetNextPageId()
  // << " GetFreeSpacePointer: " << GetFreeSpacePointer() << " GetTupleCount: " << GetTupleCount();
